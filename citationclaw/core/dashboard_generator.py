...
class DashboardGenerator:
    ...
    def _compute_stats(self, papers, total_papers, top_scholars, all_scholars):
        ...
        # 顶级奖项关键词检测
        top_award_keywords = ["诺贝尔奖", "图灵奖", "菲尔兹奖", "阿贝尔奖", "沃尔夫奖",
                              "克拉福德奖", "奈望林纳奖", "imu abacus medal", "哥德尔奖",
                              "nobel prize", "turing award", "fields medal", "abel prize",
                              "wolf prize", "crafoord prize", "abacus medal", "gödel prize",
                              "breakthrough prize", "lasker award", "shaw prize", "ieee medal of honor"]
        ...
        for s in all_scholars:
            ...
            lv = str(s.get("level", "")).lower()
            # 首先检查是否为顶级奖项得主
            if any(a in lv for a in top_award_keywords):
                level_counter["国际顶级奖项得主"] += 1
            elif "院士" in lv and "其他" not in lv and "fellow" not in lv:
                level_counter["两院院士"] += 1
            elif lv == "fellow" or "fellow" in lv:
        ...
        # 统计顶级奖项得主数量
        top_award_count = level_counter.get("国际顶级奖项得主", 0)
        ...
        return {
            ...
            "top_award_count": top_award_count,  # 国际顶级奖项得主数量
...
- 引用该论文的知名学者数：{stats['unique_scholars']}（其中院士/Fellow {stats['fellow_count']} 位，国际顶级奖项得主 {stats['top_award_count']} 位）
...
- 总知名学者数：{stats['unique_scholars']}（其中院士/Fellow {stats['fellow_count']}人，国际顶级奖项得主 {stats['top_award_count']}人）
...
color 必须从 ["teal", "sage", "amber", "violet"] 中选择（每种各一个），body 中可用 <strong> 标签加粗关键数据。"""
        ...
        return [
            ...
            {"color": "amber", "icon": "🏆", "title": "学者层次：高权威背书",
             "body": f"引用学者中包含<strong>{stats['fellow_count']}位院士/Fellow</strong>" + (f"及<strong>{stats['top_award_count']}位国际顶级奖项得主（诺贝尔奖/图灵奖/菲尔兹奖等）</strong>" if stats['top_award_count'] > 0 else "") + f"，共来自{stats['unique_scholars']}位知名学者，表明该论文获得顶尖学术圈的广泛认可。"},
...
.b-aw { background: linear-gradient(135deg, #fff8e1 0%, #ffecb3 100%); color: #b71c1c; border: 1px solid #ffc107; font-weight: 600; } /* 国际顶级奖项得主 - 金色 */
.b-ys { background: #fff3e0; color: #d4892a; border: 1px solid #f0c070; } /* 两院院士 - 橙色 */
.b-fw { background: var(--teal-light); color: var(--teal); border: 1px solid #b3d4f0; } /* Fellow - 青色 */
.b-ot { background: var(--sage-light); color: #357a62; border: 1px solid #a0d9c0; } /* 其他院士 - 绿色 */
.b-nm { background: var(--bg2); color: var(--text-muted); border: 1px solid var(--border); } /* 其他知名学者 - 灰色 */
...
"""
    ...
    def _level_badge(level):
        ...
        # 国际顶级奖项得主（最高优先级）
        top_awards = ["诺贝尔奖", "图灵奖", "菲尔兹奖", "阿贝尔奖", "沃尔夫奖", 
                      "克拉福德奖", "奈望林纳奖", "imu abacus medal", "哥德尔奖",
                      "nobel prize", "turing award", "fields medal", "abel prize",
                      "wolf prize", "crafoord prize", "nev", "abacus medal", "gödel prize",
                      "breakthrough prize", "lasker award", "shaw prize", "ieee medal of honor"]
        if any(a in lv for a in top_awards):
            return "b-aw", "国际顶级奖项得主"
        if "院士" in lv and "其他" not in lv and "fellow" not in lv:
            return "b-ys", "两院院士"
        if lv == "fellow" or "fellow" in lv:
            return "b-fw", "Fellow"
        if "其他院士" in lv:
            return "b-ot", "其他院士"
        return "b-nm", "知名学者"
...
<div class="grid-2">
  ...
  <div class="card" style="text-align:center;padding:48px 24px;color:rgba(180,210,255,0.45)">
    ...
    <div style="font-size:12px">当前选择「基础服务」，未开启引文描述搜索（Phase 4），此部分无数据。如需查看，请在首页选择「进阶服务/全面服务」后重新运行。</div>
        ...
        # Sort scholars: 国际顶级奖项得主 → 两院院士 → 其他院士 → Fellow → 其他知名学者
        _level_order_map = {"b-aw": 0, "b-ys": 1, "b-ot": 2, "b-fw": 3, "b-nm": 4}
        scholar_groups.sort(key=lambda s: _level_order_map.get(self._level_badge(s["level"])[0], 5))
...
<div class="grid-2">
  ...
  <div class="card">
    <div class="card-title"><div class="card-title-dot amber"></div>知名学者头衔层级分布（含顶级奖项）</div>
...
new Chart(document.getElementById('cFellow'), {{
  ...
  data: {{ labels: {fellow_labels}, datasets: [{{ data: {fellow_data},
    // 颜色顺序对应：其他知名学者(灰) → Fellow(青) → 其他院士(绿) → 两院院士(橙) → 国际顶级奖项得主(金)
    backgroundColor: ['rgba(160,174,192,0.5)', TEAL+'bb', SAGE+'bb', AMBER+'bb', 'rgba(255,193,7,0.85)'],
    borderColor: ['#a0aec0', TEAL, SAGE, AMBER, '#ffc107'], borderWidth: 2, hoverOffset: 8 }}] }},
